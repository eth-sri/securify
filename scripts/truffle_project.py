        ...
        for entry in os.scandir(self.build_dir):
            if entry.is_file() and entry.name.endswith(".json") and\
                ...
                if not pathlib.Path(contract_path).is_file():
                    ...
                    if node_modules_dir == None:
                        print("[!] Error: Couldn't find a source for %s" %(contract_name))
                        sys.exit(1)
                    contract_path = os.path.join(node_modules_dir, contract_path)
                ...
                result["%s:%s" %(contract_path, contract_name)] = data
